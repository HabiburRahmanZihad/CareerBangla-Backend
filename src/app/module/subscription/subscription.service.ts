...
const initiatePayment = async (user: IRequestUser, payload: { planKey?: string; couponCode?: string; referralCode?: string; customerInfo?: ICustomerInfo }) => {
    ...
    const sslczData = {
        ...
        success_url: `${envVars.BACKEND_URL}/api/v1/subscriptions/success?role=${isRecruiterSubscription ? "RECRUITER" : "USER"}`,
        fail_url: `${envVars.BACKEND_URL}/api/v1/subscriptions/fail?role=${isRecruiterSubscription ? "RECRUITER" : "USER"}`,
        cancel_url: `${envVars.BACKEND_URL}/api/v1/subscriptions/cancel?role=${isRecruiterSubscription ? "RECRUITER" : "USER"}`,
